...
function linearDlog2{T <: PolyElem}(basis::T, degExt::Integer, h0::T, h1::T,
                                   card::Nemo.fmpz, defPol::T)
    ...
    k, d, mid, B = pohligHellmanParam(card, basis, defPol)

    arr, inverses = pohligHellmanRoots(B, k, mid, defPol)

    dlogs = Array(Nemo.fmpz, charac^2+1)
    i = 0
    for y in F
        i += 1
        dlogs[i] = pohligHellmanPrec(card, mid, X-y, defPol, k , d, B, arr,
                                     inverses)[1]
    end

    i += 1
    dlogs[i], n = pohligHellmanPrec(card, basis, h1, defPol, k, d, B, arr,
                                    inverses)
...
function linearDlog2{T <: PolyElem}(basis:: Nemo.RingElem, degExt::Integer,
                                   h0::T, h1::T, card::Integer, defPol::T)
    return linearDlog2(basis, degExt, h0, h1, Nemo.fmpz(card), defPol)
end
